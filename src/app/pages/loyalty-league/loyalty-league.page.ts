...
export class LoyaltyLeaguePage implements OnInit, AfterViewInit {
  ...
  async ngOnInit() {
    const prizePool = await firstValueFrom(this._LoyaltyService.getPrizePool())
  ...
  public loyaltyLeagueMember$ = this._shs.walletExtended$.pipe(
    combineLatestWith(this._LoyaltyService.llb$, this.prizePool$),
  ...
  public ll = this._LoyaltyService.llb$.pipe(
  ...
  // public leaderBoardTable = signal([])//toSignal(this._LoyaltyService.getloyaltyBoard())
