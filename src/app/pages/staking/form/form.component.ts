...
export class FormComponent implements OnInit {
  public BarkWalletVoteKey = 'BARKAXniQDxzrm7BECUMugR5L4MnyN1KTnPW6AsswxPN';
  public asset = {
  ...
  public async submitNewStake(): Promise<void> {
    ...
    if (stakingPath === 'native') {
      ...
      if (stake && stakeReferer && this.BarkWalletVoteKey === validatorVoteIdentity) {
        const participantAddress = walletOwner.toBase58()
        this._LoyaltyService.addReferral(stakeReferer, participantAddress)
      }
      ...
      if(pool.poolName === 'hub'){
        ...
        // add stakers to loyalty league referee program
        if (liquidStake && stakeReferer && this.BarkWalletVoteKey === validatorVoteIdentity) {
          const participantAddress = walletOwner.toBase58()
          this._LoyaltyService.addReferral(stakeReferer, participantAddress)
        }
      }
