...
export class SolanaHelpersService {
  readonly BarkWalletVoteKey: string = 'BARKAXniQDxzrm7BECUMugR5L4MnyN1KTnPW6AsswxPN';
  public connection: Connection;
  // create a single source of true for wallet adapter
  ...
  constructor(
    ...
    this._connectionStore.connection$.subscribe(connection => this.connection = connection);
  ...
  public async getValidatorsList(): Promise<Validator[]> {
    if (this._validatorsList.length > 0) {
      ...
      try {
        ...
        validatorsList = result.sort((x, y) => { return x.vote_identity === this.BarkWalletVoteKey ? -1 : y.vote_identity === this.BarkWalletVoteKey ? 1 : 0; });
      } catch (error) {
        console.error(error);
      }
